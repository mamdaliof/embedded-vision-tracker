...
static GstFlowReturn brightness_callback (GstAppSink *appsink, gpointer user_data) {
  ...
  g_signal_emit_by_name (sink, "pull-sample", &sample); // pull the sample from the appsink

  /* Retrieve from the appsink buffer */
  if (sample) {
    buffer = gst_sample_get_buffer (sample); // get the buffer from the sample

    /* Map the buffer to access the pixel data */
    if (gst_buffer_map (buffer, &map, GST_MAP_READ)) {
      guint64 total_brightness = 0;
      /* In YUY2, 4 bytes represent 2 pixels. So, size / 2 equals the total pixel count. */
      guint num_pixels = map.size/2;

      /* Loop through every even byte (Y/luminance values) */
      for (gsize i = 0; i < map.size; i += 2) {
        total_brightness += map.data[i];
      }

      double avg_brightness = (double)total_brightness / num_pixels;

      /* Print the brightness and use carriage return (\r) to overwrite the same line in the terminal */
      g_print ("Current Average Brightness: %6.2f\r", avg_brightness);

      /* Unmap the buffer when done. Cleanup memory map */
      gst_buffer_unmap (buffer, &map);
    }

    gst_sample_unref (sample); // unref the sample to free memory
    return GST_FLOW_OK;
  }
      ...
      char *argv[])
{
  ...
  */
  ...
  /* Configure appsink to emit signals and connect the callback */
  g_object_set (G_OBJECT (sink), "emit-signals", TRUE, "sync", FALSE, NULL);
  g_signal_connect (sink, "new-sample", G_CALLBACK (brightness_callback), NULL);
